...
class TestOrdersEntryAgent(unittest.TestCase):
	"""Test OrdersEntryAgent orchestration."""

	def setUp(self):
		"""Set up test fixtures."""
		self.context = AgentContext()
		self.agent = OrdersEntryAgent("TestOrdersEntryAgent", context=self.context)

	def test_initialization(self):
		"""Test agent initialization."""
		self.assertEqual(self.agent.name, "TestOrdersEntryAgent")
	...
	def test_process_missing_entry_recommendations(self):
		"""Test process with no entries returns success with empty orders."""
		result = self.agent.process({})

		self.assertEqual(result["status"], "success")
		self.assertEqual(result["output"]["orders"], [])
		self.assertIn("No entry tickers", result["message"])

	def test_process_with_entries(self):
		"""Test process with valid entries dict."""
		entries = {
			"AAPL": {
				"entry_score": 80,
				"composite_score": 75,
		...
		self.context.set("entries", entries)
		...
		def mock_run_sub_agent(agent, input_data=None, *, fatal=True):
			if "OrderConstruction" in agent.name:
				self.context.set("executable_orders", executable_orders)
			return {"status": "success", "output": {}}

		with patch.object(self.agent, "_run_sub_agent", side_effect=mock_run_sub_agent):
			result = self.agent.process({})
	...
	def test_process_sub_agent_failure(self):
		"""Test process when a sub-agent raises RuntimeError."""
		self.context.set("entries", {"AAPL": {"entry_price": 150.0, "stop_loss": 145.0}})

		def mock_run_sub_agent(agent, input_data=None, *, fatal=True):
			if "CheckCash" in agent.name:
				return {"status": "success", "output": {}}
		...
		self.assertIn("Sub-agent execution failed", result["message"])

	def test_process_portfolio_name(self):
