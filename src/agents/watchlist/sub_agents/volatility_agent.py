"""Agent to filter watchlist by volatility using strategy config formula."""
...
class VolatilityAgent(Agent):
	"""Analyze volatility using formula from strategy configuration.

	Reads tickers, data_history, and volatility formula from context.
	Filters tickers where the volatility formula evaluates to True.
	Formula is a Python expression evaluated on each row of data.

	Example formula: "data['atr_14'] > 0.5 and data['atr_14'] > data['close'] * 0.01"
	"""
	...
	def process(self, input_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
		"""Analyze volatility using formula from strategy config.

		Reads 'watchlist', 'data_history', and volatility formula from context.
		Evaluates volatility formula on latest row of each ticker's data.
		...
		Returns:
			Response with volatility analysis results
		"""
		...
		if not data_history:
			self.logger.error("No data history available for volatility analysis")
			return {
				"status": "error",
				"input": input_data,
				"output": {},
				"message": "No data history available for volatility analysis"
			}

		# Get volatility formula from strategy config
		volatility_formula = None
		if strategy_config:
			watchlist_config = strategy_config.get("watchlist", {})
			volatility_config = watchlist_config.get("parameters", {}).get("volatility", {})
			volatility_formula = volatility_config.get("formula")

		if not volatility_formula:
			self.logger.debug("No volatility formula found in strategy config")
			return {
				"status": "success",
				"input": input_data,
				"output": {
					"volatility_count": len(watchlist),
					"removed_count": 0,
		...
		required_indicators = self._extract_indicators(volatility_formula)
		...
		return {
			...
			"input": input_data,
			"output": {
				"volatility_count": len(volatility_watchlist),
				"removed_count": removed_count,
				"formula_available": True
			}
		}

	def _matches_volatility_formula(self, ticker_data: Any, formula: str) -> bool:
		"""Evaluate volatility formula on latest row of ticker data.
