...
class PortfolioManager:
    ...
    def create_portfolio(
        ...
        try:
            # Normalize name to match Journal class behavior
            normalized_name = self._normalize_portfolio_name(name)

            # Create portfolio folder with normalized name
            portfolio_dir = self.portfolios_dir / normalized_name
    ...
    def get_portfolio_positions(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        for _, row in open_pos.iterrows():
            ...
            quantity = float(row["quantity"])
            avg_entry_price = float(row["avg_entry_price"])
            # Use DataHistory for prices (loads from local CSV cache, no network calls)
            # Falls back to avg_entry_price if no cached data available
