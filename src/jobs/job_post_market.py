"""Post-market job: runs the post_market step for every active bot."""
...
class JobPostMarket(Job):
	"""Runs BotFinance's post_market step for every currently active bot.

	Scheduled via config/cron.yml after the close so stale pending orders
	past their expiration_date get expired daily, without needing one cron
	entry per bot.
	"""

	def __init__(self, name: str = "job_post_market", job_dir: Optional[Path] = None, context: Optional[Any] = None):
		"""Initialize the post-market job.
	...
	def process(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
		"""Run the post_market step for every active bot.
		...
		Returns:
			...
			given bot's post_market step itself failed - failures are recorded
		...
		self.logger.info(f"JobPostMarket: found {len(active_bots)} active bot(s)")
		...
		for bot_config in active_bots:
			...
			try:
				...
				bot.activate()
				result = bot.run(params={"step": "post_market"})
				...
				if result.get("status") != STATUS_SUCCESS:
					self.logger.warning(
						f"JobPostMarket: bot '{bot_name}' returned non-success: {result.get('message')}"
					)
			except Exception as e:
				self.logger.error(f"JobPostMarket: bot '{bot_name}' failed: {e}")
		...
		self.logger.info(f"JobPostMarket: completed {succeeded}/{len(results)} bot(s) successfully")
