...
class CresusCLI(cmd2.Cmd):
	...
	def _clean_remote_hermes(self, args_str: str):
		...
		if clean_idx >= 0 and clean_idx + 1 < len(parts):
			...
			potential_host = parts[clean_idx + 1]
			if "@" in potential_host or "." in potential_host:
				remote_host = potential_host
	...
	def do_screener(self, args):
		...
		try:
			...
			if command == "list":
				...
				if portfolio and len(parts) == 2:
					# Only formula provided, portfolio is the "universe"
					formula = parts[1]
					universe_or_tickers = None
				else:
