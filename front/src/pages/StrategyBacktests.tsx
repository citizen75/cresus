...
interface StrategyBacktestsProps {
  strategyName?: string
}
...
export default function StrategyBacktests({ strategyName }: StrategyBacktestsProps) {
  const { name = '' } = useParams()
  const strategy = strategyName || name
  ...
  useEffect(() => {
    const loadBacktests = async () => {
      try {
        const result = await api.listBacktests(strategy)
  ...
  const handleDelete = async (strategyName: string, id: string) => {
    setDeleting(`${strategyName}:${id}`)
    try {
      await api.deleteBacktest(strategyName, id)
      setBacktests(backtests.filter(b => b.backtest_id !== id))
      setDeleteConfirm('')
    } catch (err) {
      ...
      <div className="relative overflow-hidden rounded-xl border border-slate-700 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 p-6 shadow-lg">
        ...
        <div className="relative flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => navigate(`/backtests/new?strategy=${encodeURIComponent(strategy)}`)}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`px-4 py-3 text-right font-bold text-sm ${
                        ...
                        {(backtest.total_return_pct ?? 0) >= 0 ? '+' : ''}{(backtest.total_return_pct ?? 0).toFixed(1)}%
                      </td>
