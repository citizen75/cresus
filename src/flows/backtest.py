...
class BacktestFlow(Flow):
	...
	def process(self, input_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
		...
		# Create backtest ID upfront (timestamp + short UUID)
		# This ensures the ID is consistent across WebSocket registration and backtest execution
		backtest_id = flow_input.get("backtest_id")  # May be pre-generated by API endpoint
		if not backtest_id:
			backtest_id = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{str(uuid.uuid4())[:8]}"

		# Set in context early for all agents and flows
		...
		# Wait for at least one WebSocket connection before starting backtest
		# This ensures frontend has time to connect and receive real-time updates
		import asyncio
		import time as time_module
		ws_manager = get_websocket_manager()

		wait_start = time_module.time()
		while time_module.time() - wait_start < 10:  # Wait up to 10 seconds
			if ws_manager.get_connection_count(backtest_id) > 0:
				self.logger.info(f"WebSocket client connected, starting backtest")
				break
			time_module.sleep(0.1)
		else:
			self.logger.info(f"No WebSocket client connected after 10s, starting backtest anyway")

		# Run backtest through agent with backtest_id (WebSocket broadcasts happen directly in BacktestAgent)
		agent_result = backtest_agent.process({
			"strategy_name": strategy_name,
			"start_date": start_date,
			"end_date": end_date,
			"lookback_days": flow_input.get("lookback_days", 365),
			"backtest_id": backtest_id,  # Pass pre-generated ID to agent
		})

		if agent_result.get("status") != "success":
			return agent_result

		# Get the actual directory from context (created by BacktestAgent)
		...
		backtest_output["backtest_dir"] = backtest_dir
		...
		# Save metrics to backtest directory
		try:
			from tools.backtest.manager import BacktestManager
			backtest_manager = BacktestManager()
			backtest_manager.save_metrics(strategy_name, backtest_id, metrics)
			self.logger.info(f"Saved metrics to backtest directory")
